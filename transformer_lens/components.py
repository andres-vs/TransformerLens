...
class BertPooler(nn.Module):
    ...
    def forward(self, resid: Float[torch.Tensor, "batch pos d_model"]) -> torch.Tensor:
        ...
        print("results after W multiplicaiton different dimensions", einsum("batch d_model_in, d_model_out d_model_in -> batch d_model_out", resid[:, 0], self.W))
        print("results dense no trans with parentheses", (einsum("batch d_model, d_model d_model -> batch d_model", resid[:, 0], self.W) + self.b))
